...
const SetCourseList = ({state, updateSearch, myInstituteHandler,
  myDegreeHandler, toggleCourse, phaseIncreaser
    ...
    if (institutes) {
      switch (phase%3) {
        case 0:
          return (
            <ScrollView>
              <List array={institutes} search={search}
                usableState={myInstitute} onPressFun={myInstituteHandler} />
            </ScrollView>);
        case 1:
          return (
            <ScrollView>
              <List array={institutes[myInstitute.index].degrees} search={search}
                usableState={myDegree} onPressFun={myDegreeHandler}/>
            </ScrollView>);
        case 2:
          return (
            <ScrollView>
              <List array={institutes[myInstitute.index].degrees[myDegree.index].courses}
                search={search} usableState={myCourses} onPressFun={toggleCourse}
                myCoursesStyleCond={true}/>
            </ScrollView>);
        default:
          return null;
      }
