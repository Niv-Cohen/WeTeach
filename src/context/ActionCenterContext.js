...
const addOffer = (dispatch) => async ({studentId, tutorId, price, Date, reqId})=>{
  try {
    await UserApi.put(`/ActionCenter/addOffer`,
        {studentId, tutorId, price, Date, reqId});
    const response =await UserApi.put(`ActinonCenter/addToMyOffers`,
        {studentId, tutorId, price, Date, reqId});
...
const addReq = (dispatch)=> async ({userId, course, subjects,
  additionalInfo, timeSlots, lessonLength})=>{
  try {
    console.log('in Add Req');
    const params = {userId, course, subjects,
      additionalInfo, timeSlots, lessonLength};
...
export const {Provider, Context} = createDataContext(actionCenterReducer,
    {createActionCenter, setLesson, addReq, addOffer, setActionCenter, getReq},
    {requests: [], actionCenter: null, errMessage: ''});
